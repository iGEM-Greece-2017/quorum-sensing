...
%    data imported from a file. The STL data can represent 3D solid or
%    planar geometry. FILENAME is a string specifying the pathname and 
%    filename of the STL file including the file extension ending in 'stl' 
%    or 'STL'. If the file is in the current directory or in a directory 
%    on the MATLAB path, the pathname can be omitted. The geometry object 
%    created is assigned to the Geometry property and a handle to the 
%    geometry is returned in G.  
%
%    Example 1:  Import a planar STL geometry and plot.
%      numberOfPDEs = 1;
%      thePde = createpde(numberOfPDEs);
%      gm = importGeometry(thePde,'PlateHolePlanar.stl')
%      pdegplot(gm, 'EdgeLabels','on')
%
%    Example 2:  Import a 3D STL geometry and plot.
%      numberOfPDEs = 1;
%      thePde = createpde(numberOfPDEs);

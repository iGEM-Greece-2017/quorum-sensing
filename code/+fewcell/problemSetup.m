...
bactRingDensity= fewcell.util.bactRingDensity(p.g.bactCenters(:,1),p.g.bactSize);
fprintf('Total bacteria: %d\n', round(sum(bactRingDensity)));
%% PDE
model= createpde(1);
...
fewcell.util.makeBactNodeCoeffs(model.Mesh,p.g.bactSize,bactRingDensity);

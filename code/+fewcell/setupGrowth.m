function setupGrowth(bactRingDensity,tlist,params)
%% Growth
...
if growth.on
  ...
  assert(~mod(params.g.nRings-growth.r0, growth.dr), '<nRings>-<r0> is not divisible by <dr>; increase <nRings>');
  growth.stepSize= sum(reshape(bactRingDensity(growth.r0+1:end),params.growth.dr,[]),1);
  if params.growth.maxRings < params.g.nRings
  ...
  params.growth.params.Nmax= sum(growth.stepSize)+inoculumSize;
  params.growth.params.Nmin= 0;   % disregard lag phase
  % Calculate growth curve
  growth.bactN= singlecell.growthCurve(inoculumSize,tlist,params.growth.params);

...
if (res > self.SolverOptions.ResidualTolerance)
    ...
    for i = 1:length(pseudocoefstruct.Coefficients)
    pseudocoefstruct.Coefficients.c{i} = 0;
    pseudocoefstruct.Coefficients.a{i} = ones(N*N,1);
    pseudocoefstruct.Coefficients.f{i} = zeros(N,1);
    end
        
    [~, a2, ~, ~, ~, a6, ~] = assemblePdeMatrices(self, ndim, p, t, pseudocoefstruct, ones(size(u(1:pdenonn))));
    %                                       c, a,            f,           u
...
tempPde.BoundaryConditions = thePde.BoundaryConditions;

function msh = generateMesh(self, varargin)    
...
%    Hmin            numeric {estimate}    Target minimum edge size on boundary (3D only)
%    GeometricOrder  linear|{quadratic}    Element geometric order 
%                                          Default GeometricOrder for 2D is linear
%    Note: For 3D meshing, if the Hmax is advertently set too low the mesh
...
% Copyright 2014-2016 The MathWorks, Inc.
    ...
    if self.IsTwoD       
       ...
       return;
        ...
        try
            [nodes, tet, cas, fas, eas, vas, Hmax, Hmin] = genmeshinternal(self.Geometry,Hmax,Hmin,geomOrder);
        catch ex
            throwAsCaller(ex);
        end       
        assoc = pde.FEMeshAssociation(tet, cas, fas, eas, vas);     
        self.Mesh = pde.FEMesh(nodes,tet,Hmax,Hmin, geomOrder,assoc);
        msh = self.Mesh;
        addlistener(msh,'ObjectBeingDestroyed',@pde.PDEModel.delistMesh); 
    end
end  
